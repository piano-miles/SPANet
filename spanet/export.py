...
class WrappedModel(pl.LightningModule):
    ...
    def apply_input_log_transform(self, sources):
        ...
        for (data, mask), name in zip(sources, self.model.event_info.input_names):
            new_data = torch.stack(
                [
                    (
                        mask * torch.log(data[:, :, i] + 1)
                        if log_transformer
                        else data[:, :, i]
                    )
                    for i, log_transformer in enumerate(
                        self.model.event_info.log_features(name)
                    )
                ],
                -1,
            )
    ...
    def forward(self, sources: List[Source]):
        ...
        if self.output_log_transform:
            assignments = list(outputs.assignments)
        ...
        embedding_vectors = (
            list(outputs.vectors.values()) if self.output_embeddings else []
        )
...
def onnx_specification(
    model, output_log_transform: bool = False, output_embeddings: bool = False
):
    input_names = []
    output_names = []
    ...
    for input_name in model.event_info.input_names:
        for input_type in ["data", "mask"]:
            ...
            dynamic_axes[current_input] = {0: "batch_size", 1: f"num_{input_name}"}
    ...
    output_names.extend(iter(model.training_dataset.regressions.keys()))
    output_names.extend(iter(model.training_dataset.classifications.keys()))
    ...
    if output_embeddings:
        ...
        for particle, products in model.event_info.product_particles.items():
            ...
            output_names.extend(
                f"{particle}/{product}/embedding_vector" for product in products
            )
...
def main(
    ...
    if int(major_version) == 2 and int(minor_version) == 0:
        raise RuntimeError(
            "ONNX export with Torch 2.0.x is not working. Either install 2.1 or 1.13."
        )
    ...
    wrapped_model = WrappedModel(
        model, input_log_transform, output_log_transform, output_embeddings
    )
    ...
    input_names, output_names, dynamic_axes = onnx_specification(
        model, output_log_transform, output_embeddings
    )
    ...
    if not input_log_transform:
        print(
            "WARNING -- No input log transform! User must apply log transform manually. -- WARNING"
        )
    print("-" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_file", type=str, help="Name to output the ONNX model to."
    )

    parser.add_argument(
        "-g", "--gpu", action="store_true", help="Trace the network on a gpu."
    )
