...
class SymmetricAttentionFull(SymmetricAttentionBase):
    ...
    def make_contraction(self):
        ...
        operations = map(
            lambda x: batch_index_name + "".join(x),
            zip(input_index_names, weights_index_names),
        )
        ...
        shapes = [
            (self.batch_size, self.DEFAULT_JET_COUNT, self.features)
        ] * self.degree
    ...
    def forward(self, x: Tensor, padding_mask: Tensor, sequence_mask: Tensor) -> Tensor:
        ...
        symmetric_weights = symmetric_tensor(
            self.weights, self.no_identity_permutations
        )
