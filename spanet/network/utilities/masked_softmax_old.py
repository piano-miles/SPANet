...
@torch.jit.script
def masked_log_sum_exp(
    x: Tensor, mask: Tensor, dim: int = 1, epsilon: Tensor = tensor(1e-7)
) -> Tensor:
...
@torch.jit.script
def training_masked_log_softmax(
    x: Tensor, mask: Tensor, dim: int = 1, epsilon: Tensor = tensor(1e-6)
) -> Tensor:
    normalization_term = masked_log_sum_exp(x, mask, dim, epsilon)
...
@torch.jit.script
def masked_softmax(
    x: Tensor, mask: Tensor, dim: int = 1, eps: Tensor = tensor(1e-6, dtype=torch.float)
) -> Tensor:
