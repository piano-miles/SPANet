...
class MultiOutputBranchLinear(nn.Module):
    ...
    def __init__(self, options: Options, num_layers: int, num_outputs: Tensor):
        ...
        self.shared_layers = BranchLinear(
            options, max(self.num_layers - 1, 1), self.hidden_dim, batch_norm=False
        )

        self.output_layers = nn.ModuleList(
            BranchLinear(options, 1, output_dim.item()) for output_dim in num_outputs
        )
