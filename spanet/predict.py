...
def create_hdf5_output(
    ...
    with h5py.File(output_file, "w") as output:
        ...
        for event_particle in dataset.event_info.event_particles:
            for i, product_particle in enumerate(
                dataset.event_info.product_particles[event_particle]
            ):
                output.create_dataset(
        ...
        for name, classification in evaluation.classifications.items():
            output.create_dataset(
                f"{SpecialKey.Classifications}/{name}", data=classification
            )
...
def main(
    ...
    if output_vectors:
        evaluation, full_outputs = evaluate_on_test_dataset(
            model, return_full_output=True, fp16=fp16
        )
    else:
        evaluation = evaluate_on_test_dataset(
            model, return_full_output=False, fp16=fp16
        )
        full_outputs = None
