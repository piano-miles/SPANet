...
from spanet.dataset.types import Evaluation  # NOTE: Unused?
...
def create_table(table: dict, full_row: bool = False, event_type: str = None) -> None:
    table = {k: list(map(formatter, v)) for k, v in table.items()}
    ...
    if min_len < max_len:
        stderr.write(
            "Table is out of shape, please make sure all columns have the same length."
        )
        stderr.flush()
        return
    ...
    for key in key_list:
        ...
        heading_line = (
            key + (" " * (rc_len + (additional_spacing + 1)))
        ) + heading_separator
    ...
    for n in range(len(value_list)):
        ...
        line = ("{:{}} " + " " * len(key)).format(
            value_list[next_idx], str(rc_len + additional_spacing)
        ) + horizontal_split
...
def display_latex_table(
    results: Dict[str, Any], jet_limits: List[str], clusters: List[str]
):
    ...
    print(r"\hline")
    print(
        r"& $N_\mathrm{jets}$ & Event Proportion  & Jet Proportion & Event Purity & ",
        end="",
    )
    ...
    event_types = set(
        map(
            lambda x: x.split("/")[0],
            filter(lambda x: "/" in x, next(iter(results.values()))),
        )
    )
    for event_type in sorted(event_types):
        ...
        for jet_limit in jet_limits:
            particle_keys = [
                key.split("/")[1]
                for key in results[jet_limit]
                if event_type in key and "event" not in key
            ]
            ...
            columns["Event Proportion"].append(
                results[jet_limit][f"{event_type}/event_proportion"]
            )
            columns["Jet Proportion"].append(results[jet_limit]["event_jet_proportion"])
            columns["Event Purity"].append(
                results[jet_limit][f"{event_type}/event_purity"]
            )
...
def evaluate_predictions(
    ...
    jet_limits.extend((f">= {minimum_jet_count + lines}", None))
    results = {}
    for jet_limit_name in jet_limits:
        ...
        if jet_limit_name is not None:
            jet_limit = eval(f"num_vectors {jet_limit_name}")
        ...
        results[jet_limit_name] = evaluator.full_report_string(
            limited_predictions, limited_targets, limited_masks
        )
        results[jet_limit_name]["event_jet_proportion"] = (
            1.0 if jet_limit_name is None else jet_limit.mean()
        )
...
def main(
    ...
    targets = [
        assignment[0].cpu().numpy()
        for assignment in model.testing_dataset.assignments.values()
    ]
    masks = [
        assignment[1].cpu().numpy()
        for assignment in model.testing_dataset.assignments.values()
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-g", "--gpu", action="store_true", help="Evaluate network on the gpu."
    )
    ...
    parser.add_argument(
        "-tex", "--latex", action="store_true", help="Output a latex table."
    )
